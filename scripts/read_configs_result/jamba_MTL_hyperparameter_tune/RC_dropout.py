...
aug = [str(round(i, 1)).replace('.', '_') for i in np.arange(0.1, 0.8, 0.1).tolist()]


PARAMETER_NAME= f'Dropout Rate'
...
for index, val in enumerate(aug):
    ...
    model_config_dict[val] = [f"jamba_20240717_{seeds[i]}NCV_JambaV2_AUG_0_layers_1_best_others_1-Task_depression_wGNN_wMLP_clipnorm_1_weightDecay_0_004_num_layer_0_w_mamba_dropout_{val}_patience_20" for i in range(len(seeds))]
    # head
