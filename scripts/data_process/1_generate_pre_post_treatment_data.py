...
def generate_response_label(scores):
    ...
    score_change_percentage = (
        scores[:, 1]-scores[:, 0])/scores[:, 0]  # score changes
...
output_pre_and_post_treatment_path = mainfold + \
    '/pre_post_treatment_hamd_reduction_50/'


# read the hbo and hbr in pre-treatment time and post treatment time
hbo_base, hbo_t8, hbr_base, hbr_t8 = read_hb(
    pre_and_post_datapath, pre_and_post_labelpath)
...
hbo_hbr_base_t8 = np.concatenate(
    (hbo_hbr_base[..., np.newaxis], hbo_hbr_t8[..., np.newaxis]), axis=3)
...
pre_post_data, label = balance_label_0and1(
