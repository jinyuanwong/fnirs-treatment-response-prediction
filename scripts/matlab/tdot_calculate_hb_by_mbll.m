function varargout = tdot_calculate_hb_by_mbll(varargin)
% Calculate hemoglobin concentration by MBLL
% 
% @=============================================================================
% This function is developed by Optical Imaging Lab, BME, NUS
%
% Authors: Li Zhifei, 2020
% =============================================================================@
...
%% ===== RUN =====
% Args:
%   - nirs_psig:        channels x wavelengths x times
%   - wavelengths:      array of double, e.g. [795 850]
%   - normalize_method: 'mean', 'median'
%   - pair_distances:   channels x 1
%
function nirs_hb_p = Run(nirs_psig, wavelengths, normalize_method, pair_distances) %#ok<DEFNU>
    do_plp_corr = false;
    nChannel    = size(nirs_psig, 1);
    nWL         = size(nirs_psig, 2);
    nTime       = size(nirs_psig, 3);
    
    for ipair=1:size(nirs_psig, 1)
        hb_extinctions = get_hb_extinctions(wavelengths); % cm^-1.l.mol^-1
        ...
        % Calculate HbO, HbR, HbT
        nirs_hb_p(1:2, ipair, :)    = 1 ./ pair_distances(ipair) .* ...
                                        pinv(hb_extinctions) * delta_od_ppf_fixed; %mol.l^-1
        nirs_hb_p(3, ipair, :)      = sum(nirs_hb_p(1:2, ipair, :), 1);
    end
end
...
% whole blood is x=150 g Hb/liter.
    ...
    for ihb=1:2
        for iwl=1:nb_wavelengths
            hb_extinctions(iwl, ihb) = interp1(double( extinction_hbo_hbr_ref(:,1) ), ...
                                               double( extinction_hbo_hbr_ref(:,ihb+1) ), ...
                                               double( wavelengths(iwl) ));
        end
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% - intensity_2d: channel_wl x times
% - nirs_psig: channels x wavelengths x times
function nirs_psig = Format_NIRS_2D_to_3D(intensity_2d)
    nCHWL       = size(intensity_2d, 1);
    nChannel    = nCHWL / 2;
    nTime       = size(intensity_2d, 2);
    % wavelength - 795
    nirs_psig(:, 1, :) = intensity_2d(1:nChannel, 1:nTime);
    % wavelength - 850
    nirs_psig(:, 2, :) = intensity_2d((nChannel + 1) : nCHWL, 1:nTime);
end
