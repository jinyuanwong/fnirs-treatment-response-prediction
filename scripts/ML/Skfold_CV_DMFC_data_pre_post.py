...
output_fold = 'results/ML_results/DMFC/Skfold_CV_DMFC/pre_post_treatment_hamd_reduction_50'
if not os.path.exists(output_fold):
    os.makedirs(output_fold)
    
input_path = 'allData/prognosis/DMFC/pre_post_treatment_hamd_reduction_50'
...
def stratified_k_fold_cross_validation_with_holdout(data, label):
    ...
    X_test = np.concatenate(
        (pos[:holdout_pos_num], neg[:holdout_neg_num]), axis=0)
    Y_test = np.concatenate(
        (np.ones(holdout_pos_num), np.zeros(holdout_neg_num)), axis=0)
    ...
    Y_train = np.concatenate(
        (np.ones(train_val_pos.shape[0]), np.zeros(train_val_neg.shape[0])), axis=0)
...
def pre_post_read_hb_label(HB_TYPE, combine_type, fold):
    # read data
...
for name, model in models.items():
    ...
    for num_time in range(5):
        ...
        for HB_TYPE in ['HbO', 'HbR', 'HbT']:
            ...
            hb_2d, label = pre_post_read_hb_label(
                HB_TYPE, 'substract', input_path)
...
#             accuracies = {}
#             num_of_k_fold = int((label == 1).sum() * 2 / 3 / 2)
#             X_train, y_train, X_test, y_test = stratified_k_fold_cross_validation_with_holdout(
#                 hb_2d, label)
