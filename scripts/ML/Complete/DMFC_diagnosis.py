...
from utils.fnirs_utils import AutoML_data_dict_for_DMFC
...
def get_avg_pretreatment_data_label_demo_for_dmfc(hb_type=0):
    hb_type_dict = {
        'hbo': 0,
        'hbr': 1,
        'hbt': 2}
    FOLD = 'allData/diagnosis/'
    pretreatment_path = FOLD + 'fc_data.npy'
    ...
    return avg_pretreatment_data, pretreatment_label, pretreatment_demo

def start():
    avg_pretreatment_data, pretreatment_label, pretreatment_demo = get_avg_pretreatment_data_label_demo_for_dmfc(hb_type=0)
...
def automl(model_name, hb_type, save_fold, iteration_time):
    ...
    avg_pretreatment_data, pretreatment_label, pretreatment_demo = get_avg_pretreatment_data_label_demo_for_dmfc(hb_type=hb_type)

    for itr in range(iteration_time):
        seed = int(time.time())
        res_metrics, para, _ = specify_model_and_train(avg_pretreatment_data, pretreatment_label, model_name, seed)
    ...
    save_autodl(DATA_to_save, save_fold + '/DMFC_diagnosis.csv')
...
def predict_based_on_automl(csv_file='results/ML_results/AutoML/DMFC_diagnosis.csv', ref_param='F1_score'):

    # print(f'Average every 10 timepoints -> shape: {avg_pretreatment_data.shape}')
    model_name, hb_type, seed = get_best_seed_from_automl(csv_file, ref_param)

    avg_pretreatment_data,pretreatment_label,pretreatment_demo = get_avg_pretreatment_data_label_demo_for_dmfc(hb_type)
