...
from utils_simple_prediction import add_task_change_data, load_data_for_classification, load_data_for_partial_response_prediction, add_cgi, add_mddr, set_path, load_task_change_data, save_model_seed
import time
...
def train(num_of_repeat, partial_response, random_seed = 42): 
    ...
    for weight_0 in [i/10 for i in range (2, 10)]:#[None]: # np.arange(0.2, 1, 0.1): #[None]:# [0.95, 0.9, 0.85, 0.8, 0.75, 0.7, 0.65, 0.6, 0.55, 0.5]:
        modalities = ['fnirs only'] #  ['clinical only', 'fnirs only', 'clinical + fnirs'] #   ['clinical only', 'fnirs only', 'clinical + fnirs'] #      ['clinical only', 'clinical + fnirs']# 
...
if __name__ == "__main__":
    ...
    train(num_of_repeat=5, partial_response=partial_response, random_seed=time_seed) # random_seed=42
