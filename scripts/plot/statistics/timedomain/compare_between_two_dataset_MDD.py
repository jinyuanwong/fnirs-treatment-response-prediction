...
MDD_1 = DATA1[LABEL1==1] 
MDD_2 = DATA2[LABEL2==1]

COMP_MDD = np.concatenate((MDD_1, MDD_2), axis=0)
COMP_MDD_LABEL = np.concatenate((np.zeros((MDD_1.shape[0],)), np.ones((MDD_2.shape[0],))), axis=0)

pre_post_data_combine_type = 'substract'

output_fold = 'FigureTable/statics/timedomain/compare_two_dataset_MDD'
...
for fig_name in name_of_input:
    ...
    MDD = HbO[label==1]
    plt.figure() 
    plt.plot(np.mean(HC, axis=(0,2)), label=f"Fabeha's MDD")
    plt.plot(np.mean(MDD, axis=(0,2)), label=f"Gabreille's MDD")
    plt.legend()
    plt.title('Comparison of two datasets within MDD subjects')
