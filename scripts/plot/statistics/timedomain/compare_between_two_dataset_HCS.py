...
DATA1 =  np.load('allData/dataset_514/fabeha_num_of_sub_374/hb_data.npy')
LABEL1 =  np.load('allData/dataset_514/fabeha_num_of_sub_374/label.npy')

DATA2 = np.load('allData/dataset_514/gabrielle_num_of_sub_141/hb_data.npy')
LABEL2 = np.load('allData/dataset_514/gabrielle_num_of_sub_141/label.npy')


HCS_1 = DATA1[LABEL1==0] 
HCS_2 = DATA2[LABEL2==0]

COMP_HC = np.concatenate((HCS_1, HCS_2), axis=0)
COMP_HC_LABEL = np.concatenate((np.zeros((HCS_1.shape[0],)), np.ones((HCS_2.shape[0],))), axis=0)

pre_post_data_combine_type = 'substract'

output_fold = 'FigureTable/statics/timedomain/compare_two_dataset_HCS'
...
for fig_name in name_of_input:
    ...
    print(f"label shape: {label.shape} | LABEL1 shape: {LABEL1.shape}")
    ...
    MDD = HbO[label==1]
    plt.figure() 
    plt.plot(np.mean(HC, axis=(0,2)), label=f"Fabeha's HCS")
    plt.plot(np.mean(MDD, axis=(0,2)), label=f"Gabreille's HCS")
    plt.legend()
    plt.title('Comparison of two datasets within HCs')
