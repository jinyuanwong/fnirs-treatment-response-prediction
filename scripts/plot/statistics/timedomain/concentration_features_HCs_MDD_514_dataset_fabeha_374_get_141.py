...
FOLD = 'allData/dataset_514/fabeha_num_of_sub_374'
...
output_fold = 'FigureTable/statics/timedomain/dataset_514_from_fabeha_num_of_sub_374_get_141'
...
for fig_name in name_of_input:
    ...
    indices = np.random.choice(LABEL.shape[0], 141, replace=False)
    data = data[indices]
    label = label[indices]
    ...
    MDD = HbO[label==1]
    plt.figure() 
    plt.plot(np.mean(HC, axis=(0,1))[:,0], label=f"HCs {HC.shape[0]}")
    plt.plot(np.mean(MDD, axis=(0,1))[:,0], label=f"MDDs {MDD.shape[0]}")
    plt.legend()
