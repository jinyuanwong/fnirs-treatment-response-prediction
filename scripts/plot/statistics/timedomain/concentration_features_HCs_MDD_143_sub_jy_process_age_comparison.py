...
def plot_save_data():
    ...
    for fig_name in name_of_input:
        ...
        show_hb_type(HbO, label, 'HbO', fig_name + '_HbO' + '_subject' + str(data.shape[0]), 300, 900)
        # For pre - treatment HAMD reduction 50 - HbR
        show_hb_type(HbR, label, 'HbR', fig_name + '_HbR' + '_subject' + str(data.shape[0]), 300, 900)
        # For pre - treatment HAMD reduction 50 - HbT 
        show_hb_type(HbT, label, 'HbT', fig_name + '_HbT' + '_subject' + str(data.shape[0]), 300, 900)

titles = ['With age correction', 'Without age correction']

for i, val in enumerate(['with_age', 'without_age']):
    
    fold = '/Users/shanxiafeng/Documents/Project/Research/fnirs-prognosis/code/fnirs-treatment-response-prediction/allData/diagnosis/matlab_process_MBLL'
    DATA =  np.load(fold + f'/{val}.npy') # np.load('allData/diagnosis/hb_data.npy')
    LABEL = np.load(fold + '/label.npy') #  np.load('allData/diagnosis/label.npy')
    
    output_fold = f'FigureTable/statics/timedomain/baseline_dataset_jy_AGE_comparison/{val}'
