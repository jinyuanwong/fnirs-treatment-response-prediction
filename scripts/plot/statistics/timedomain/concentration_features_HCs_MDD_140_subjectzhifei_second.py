...
DATA =  np.load('allData/zf_prognosis/prep_diagnosis/avg_hb_data.npy')# np.load('/Users/shanxiafeng/Documents/Code/python/fnirs_DL/JinyuanWang_pythonCode/allData/Output_npy/twoDoctor/HbO-All-HC-MDD/correct_channel_data.npy') # np.load('allData/diagnosis/hb_data.npy')
LABEL = np.load('allData/zf_prognosis/prep_diagnosis/label.npy') # np.load('/Users/shanxiafeng/Documents/Code/python/fnirs_DL/JinyuanWang_pythonCode/allData/Output_npy/twoDoctor/HbO-All-HC-MDD/label.npy') #  np.load('allData/diagnosis/label.npy')

pre_post_data_combine_type = 'substract'

output_fold = 'FigureTable/statics/timedomain/numOfSubject_140_processedByZhifei'
...
for fig_name in name_of_input:
    ...
    show_hb_type(HbR, label, 'HbR', fig_name + '_HbR' + '_subject' + str(data.shape[0]), 10, 70)
    # For pre - treatment HAMD reduction 50 - HbT 
    show_hb_type(HbT, label, 'HbT', fig_name + '_HbT' + '_subject' + str(data.shape[0]), 10, 70)
