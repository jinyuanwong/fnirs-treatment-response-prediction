...
def loop_iteration_find_best_performance_id(total_itr, model, file_name, verbose=True):
    read_path = folder_path + '/' + filename + '/' + 'fold-'
    best_acc = 0
    best_itr = 0
    final_wrong_index = []
...
def refer_val_get_test(total_itr, model):
    ...
    read_path = read_path = folder_path + '/' + filename + '/' + 'fold-'
...
def generate_md_table(K_fold):
    k_fold_name = f'{K_fold}-fold with hold out'
    # print(f'| {k_fold_name} | Testing Set |             |             |             | Validation Set |             |             |             |')
    # print('|------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|')
    # print('| Model Name | Accuracy | Sensitivity | Specificity | F1 Score | Accuracy | Sensitivity | Specificity | F1 Score |')


    # print('| Dataset | Model Name | Accuracy | Sensitivity | Specificity | F1 Score |')
    # print('|------------|------------|----------|-------------|-------------|----------|')
    print(f'| {model}(k={K_fold})   |', end='')
