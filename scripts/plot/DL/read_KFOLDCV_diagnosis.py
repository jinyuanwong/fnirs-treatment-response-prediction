...
model = 'gnn_transformer'  # comb_cnn or cnn_transformer gnn_transformer gnn
...
model_params = 'v1l1_rate_0.01_l2_rate_0.01_d_model_16_batch_size_64_n_layers_6'

validation_method = 'Stratified_10_fold_CV'  # 'LOOCV' or 'k_fold' LOO_nested_CV

output_fold = f'FigureTable/DL/timedomain'

if not os.path.exists(output_fold):
    os.makedirs(output_fold)

y_test_path = f'allData/diagnosis'
...
def get_val_metrics(model):
    ...
    for loo in range(total_subjects):
        ...
        for cv_fold in range(num_of_cv_folds):
            read_path = f"results/{model}/{model_params}/{validation_method}/fold-{cv_fold}/val_acc.txt"
...
def get_y_pred_test(model):
    ...
    for loo in range(total_subjects):
        ind_loo_folds =[]
        for cv_fold in range(num_of_cv_folds):
            read_path = f"results/{model}/{model_params}/{validation_method}/fold-{cv_fold}/test_acc.txt"
