...
class GraphAttention(Layer):
    ...
    def build(self, input_shape):
        ...
        for head in range(self.attn_heads):
            ...
            # Attention kernels
            attn_kernel_self = self.add_weight(shape=(self.F_, N),
                                               initializer=self.attn_kernel_initializer,
                                               regularizer=self.attn_kernel_regularizer,
                                               constraint=self.attn_kernel_constraint,
                                               name='attn_kernel_self_{}'.format(head),)
            attn_kernel_neighs = self.add_weight(shape=(self.F_, N),
                                                 initializer=self.attn_kernel_initializer,
                                                 regularizer=self.attn_kernel_regularizer,
                                                 constraint=self.attn_kernel_constraint,
    ...
    def call(self, inputs, adjacency_matrix):
        ...
        A = adjacency_matrix  # Adjacency matrix (N x N)
        ...
        for head in range(self.attn_heads):
            ...
            attn_for_self = tf.matmul(features, attention_kernel[0])  # (N x N), [a(Wh_i, Wh_i)]
            attn_for_neighs = tf.matmul(features, attention_kernel[1])  # (N x N), [a(Wh_i, Wh_j)]

            # Attention head a(Wh_i, Wh_j) = a^T [[Wh_i], [Wh_j]]
            dense = attn_for_self + tf.transpose(attn_for_neighs, perm=[0, 2, 1])  # (batch_size, 1, N)

            # Add nonlinearty
            dense = tf.nn.leaky_relu(dense, alpha=0.2)  # LeakyReLU with negative input slope, alpha=0.2
