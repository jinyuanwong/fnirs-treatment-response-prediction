...
class Classifier_GraphSAGE_Transformer():
    def __init__(self, output_directory, callbacks, input_shape, epochs, sweep_config, info):
        ...
        FFN_units = parameter['FFN_units'] # sweep_config['FFN_units'] if sweep_config else 256
        n_heads = sweep_config['n_heads'] if sweep_config else 4  # 5  # random.choice([4, 8])  # 2
        ...
        outputs = layers.LSTM(units=lstm_unit, return_sequences=True, return_state=False)(outputs)
