...
class Classifier_ALSTM_concatenate_RESNET:
    ...
    def ResNet(self, channel):
        ...
        conv_x = kl.Conv1D(filters=16, kernel_size=8, padding='same')(channel)
        conv_x = kl.BatchNormalization()(conv_x)
        conv_x = kl.Activation(self.activation)(conv_x)

        conv_y = kl.Conv1D(filters=16, kernel_size=8, padding='same')(conv_x)
        conv_y = kl.BatchNormalization()(conv_y)
        conv_y = kl.Activation(self.activation)(conv_y)

        conv_z = kl.Conv1D(filters=16, kernel_size=8, padding='same')(conv_y)
        conv_z = kl.BatchNormalization()(conv_z)

        # expand channels for the sum
        shortcut_y = kl.Conv1D(filters=16, kernel_size=8, padding='same')(channel)
        ...
        conv_x = kl.Conv1D(filters=32, kernel_size=5, padding='same')(output_block_1)
        conv_x = kl.BatchNormalization()(conv_x)
        conv_x = kl.Activation(self.activation)(conv_x)

        conv_y = kl.Conv1D(filters=32, kernel_size=5, padding='same')(conv_x)
        conv_y = kl.BatchNormalization()(conv_y)
        conv_y = kl.Activation(self.activation)(conv_y)

        conv_z = kl.Conv1D(filters=32, kernel_size=5, padding='same')(conv_y)
        conv_z = kl.BatchNormalization()(conv_z)

        # expand channels for the sum
        shortcut_y = kl.Conv1D(filters=32, kernel_size=5, padding='same')(output_block_1)
        shortcut_y = kl.BatchNormalization()(shortcut_y)
        ...
        conv_x = kl.Conv1D(filters=32, kernel_size=3, padding='same')(output_block_2)
        conv_x = kl.BatchNormalization()(conv_x)
        conv_x = kl.Activation(self.activation)(conv_x)

        conv_y = kl.Conv1D(filters=32, kernel_size=3, padding='same')(conv_x)
        conv_y = kl.BatchNormalization()(conv_y)
        conv_y = kl.Activation(self.activation)(conv_y)

        conv_z = kl.Conv1D(filters=32, kernel_size=3, padding='same')(conv_y)
        conv_z = kl.BatchNormalization()(conv_z)

        # no need to expand channels because they are equal
        shortcut_y = kl.Conv1D(filters=32, kernel_size=3, padding='same')(
    ...
    def ALSTM(self,Channel):
        ...
        m = kl.Dropout(0.5)(m)
        e = kl.Dense(1, activation=self.activation)(m)
        # e = kl.Flatten()(e)
        a = kl.Activation('softmax')(e)
        temp = kl.RepeatVector(1)(a)
        temp = kl.Permute([2, 1])(temp)
    ...
    def ALSTM_concatanate_RESNET(self,Channel):

        alstm = self.ALSTM(Channel)

        #FCN
        resnet = self.ResNet(Channel)


        alstm_resnet = kl.concatenate([alstm,resnet])


        m = km.Model(inputs=Channel, outputs=alstm_resnet)
    ...
    def build_model(self, input_shape, nb_classes):
        ...
        if self.useCombinationModel == True:
            ...
            m0 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m1 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m2 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m3 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m4 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m5 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m6 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m7 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m8 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m9 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m10 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m11 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m12 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m13 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m14 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m15 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m16 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m17 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m18 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m19 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m20 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m21 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m22 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m23 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m24 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m25 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m26 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m27 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m28 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m29 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m30 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m31 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m32 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m33 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m34 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m35 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m36 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m37 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m38 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m39 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m40 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m41 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m42 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m43 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m44 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m45 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m46 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m47 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m48 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m49 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m50 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
            m51 = self.ALSTM_concatanate_RESNET(kl.Input(input_shape))
