...
class Classifier_Inception:
    ...
    def __init__(self, output_directory, result_name, input_shape, nb_classes, nb_epochs, hyperparameter,useCombinationModel, verbose,
                 build=True):
    ...
    def _inception_module(self, input_tensor, stride=1):
        ...
        input_inception = kl.Conv1D(filters=1, kernel_size=1, padding='same', activation=self.activation,
                                    use_bias=False)(
            input_tensor)
        conv_list = []
        for i in range(len(self.inception_kernelSize)):
            conv_list.append(kl.Conv1D(filters=1, kernel_size=self.inception_kernelSize[i],
                                       strides=stride, padding=self.padding, activation=self.activation,
                                       use_bias=False)(
                input_inception))
    ...
    def Inception(self, channel):
        ...
        m = km.Model(inputs=channel, outputs=x)
    ...
    def build_model(self, input_shape, nb_classes):
        ...
        if self.useCombinationModel == True:
            ...
            m0 = self.Inception(kl.Input(input_shape))
            m1 = self.Inception(kl.Input(input_shape))
            m2 = self.Inception(kl.Input(input_shape))
            m3 = self.Inception(kl.Input(input_shape))
            m4 = self.Inception(kl.Input(input_shape))
            m5 = self.Inception(kl.Input(input_shape))
            m6 = self.Inception(kl.Input(input_shape))
            m7 = self.Inception(kl.Input(input_shape))
            m8 = self.Inception(kl.Input(input_shape))
            m9 = self.Inception(kl.Input(input_shape))
            m10 = self.Inception(kl.Input(input_shape))
            m11 = self.Inception(kl.Input(input_shape))
            m12 = self.Inception(kl.Input(input_shape))
            m13 = self.Inception(kl.Input(input_shape))
            m14 = self.Inception(kl.Input(input_shape))
            m15 = self.Inception(kl.Input(input_shape))
            m16 = self.Inception(kl.Input(input_shape))
            m17 = self.Inception(kl.Input(input_shape))
            m18 = self.Inception(kl.Input(input_shape))
            m19 = self.Inception(kl.Input(input_shape))
            m20 = self.Inception(kl.Input(input_shape))
            m21 = self.Inception(kl.Input(input_shape))
            m22 = self.Inception(kl.Input(input_shape))
            m23 = self.Inception(kl.Input(input_shape))
            m24 = self.Inception(kl.Input(input_shape))
            m25 = self.Inception(kl.Input(input_shape))
            m26 = self.Inception(kl.Input(input_shape))
            m27 = self.Inception(kl.Input(input_shape))
            m28 = self.Inception(kl.Input(input_shape))
            m29 = self.Inception(kl.Input(input_shape))
            m30 = self.Inception(kl.Input(input_shape))
            m31 = self.Inception(kl.Input(input_shape))
            m32 = self.Inception(kl.Input(input_shape))
            m33 = self.Inception(kl.Input(input_shape))
            m34 = self.Inception(kl.Input(input_shape))
            m35 = self.Inception(kl.Input(input_shape))
            m36 = self.Inception(kl.Input(input_shape))
            m37 = self.Inception(kl.Input(input_shape))
            m38 = self.Inception(kl.Input(input_shape))
            m39 = self.Inception(kl.Input(input_shape))
            m40 = self.Inception(kl.Input(input_shape))
            m41 = self.Inception(kl.Input(input_shape))
            m42 = self.Inception(kl.Input(input_shape))
            m43 = self.Inception(kl.Input(input_shape))
            m44 = self.Inception(kl.Input(input_shape))
            m45 = self.Inception(kl.Input(input_shape))
            m46 = self.Inception(kl.Input(input_shape))
            m47 = self.Inception(kl.Input(input_shape))
            m48 = self.Inception(kl.Input(input_shape))
            m49 = self.Inception(kl.Input(input_shape))
            m50 = self.Inception(kl.Input(input_shape))
            m51 = self.Inception(kl.Input(input_shape))
