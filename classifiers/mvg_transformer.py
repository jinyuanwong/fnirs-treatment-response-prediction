...
class MultiViewAggregator(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.num_views = input_shape[-1]  # [0,1,3], [0,2,4]
        # node_target dimension is considered as feature dimension
    ...
    def call(self, inputs):
        ...
        attention_scores = tf.tensordot(
            inputs, self.weight_vector, axes=[3, 0])
        ...
        attention_coefficients_expanded = tf.expand_dims(
            attention_coefficients, axis=-1)

        # Multiply the inputs with the attention coefficients and sum over the views
        weighted_sum = tf.reduce_sum(
            inputs * attention_coefficients_expanded, axis=-1)
...
class MVGCN_CNN_VERSION(tf.keras.Model):
    def __init__(self,
                 kernel_cnn,
                 activation='relu'):
        super(MVGCN_CNN_VERSION, self).__init__()

        # self.W = layers.Dense(units=d_model)
        self.W = layers.Conv2D(filters=1, kernel_size=[1, kernel_cnn])
    ...
    def call(self, inputs, adj):
        ...
        # # if self.W = layer.Dense()
        # inputs_features = self.W(inputs)

        # if self.W = layer.conv2d()
        inputs_features = self.W(inputs[..., tf.newaxis])
...
class Classifier_MVG_Transformer():
    def __init__(self, output_directory, callbacks, input_shape, epochs, sweep_config, info):
        ...
        self.batch_size = parameter['batch_size']# sweep_config['batch_size'] if sweep_config else 128
        ...
        # random.choice([12, 8, 16])
        n_layers = parameter['n_layers']
        # random.choice([64, 128, 256, 512])  # 512, 64, 128,
        # sweep_config['FFN_units'] if sweep_config else 256
        FFN_units = parameter['FFN_units']
        # 5  # random.choice([4, 8])  # 2
        n_heads = sweep_config['n_heads'] if sweep_config else 4
        ...
        l2_rate = parameter['l2_rate']  # 0.001
        num_class = 2  # 2
        num_view = input_shape[-1]
        # if num_view is 2 -> [0,1]
        # if num_view is 5 -> [0, 1, 3]
        selected_view_1 = [0] + [2 * n + 1 for n in range(num_view//2)] 

        # if num_view is 2 -> [0]
        # if num_view is 5 -> [0, 2 ,4]
        selected_view_2 = [2 * n for n in range(0, (num_view+1)//2)] 

        print(f'num_view {num_view}'* 100)
        lr_factor = parameter['lr_factor']  # 1
        ...
        input_adj = tf.keras.Input(
            shape=(input_shape[1], input_shape[1], num_view))
        # output_1 = MultiChannelCNN(padding= 'same', kernel_size= [8, 3], activation='sigmoid')(inputs[..., tf.newaxis])
        # kernel_cnn = sweep_config['kernel_cnn'] if sweep_config else 3
        select_view_connectivity = tf.gather(
            input_adj, tf.constant(selected_view_1), axis=-1)
        select_view_coherence = tf.gather(
            input_adj, tf.constant(selected_view_2), axis=-1)
        output_1 = MVGCN(d_model)(inputs, select_view_connectivity)

        output_2 = MVGCN(d_model)(inputs, select_view_coherence)

        # if sweep_config:
        for _ in range(parameter['mvg_layers_depth']-1):
            output_1 = MVGCN(d_model)(output_1, select_view_connectivity)
            output_2 = MVGCN(d_model)(output_2, select_view_coherence)
        ...
        output_1 = Transformer(dropout_rate,
                               n_layers,
                               FFN_units,
                               n_heads,
                               activation)(output_1)

        output_2 = Transformer(dropout_rate,
                               n_layers,
                               FFN_units,
                               n_heads,
                               activation)(output_2)
