...
class Classifier_MGM_Transformer():
    def __init__(self, output_directory, callbacks, input_shape, epochs, sweep_config, info):
        ...
        self.batch_size = sweep_config['batch_size'] if sweep_config else 8
        ...
        lr_factor = parameter['lr_factor']
        lstm_unit = parameter['lstm_unit']
        feature_dim = input_shape[-1]  # 125
        ...
        conc_outputs = []
        outputs = GCN(d_model)(inputs, input_adj[..., 0])
        outputs = ClsPositionEncodingLayer(
            input_channel=input_shape[1], kenerl_size=kernel_size[0], strides=stride_size[0], d_model=d_model, dropout_rate=dropout_rate, name='CLS_pos_encoding_1')(outputs)
        outputs = Transformer(dropout_rate,
                              n_layers,
                              FFN_units,
                              n_heads,
                              activation)(outputs)
        outputs = layers.Dense(d_model, activation=activation)(outputs)
        conc_outputs.append(tf.expand_dims(outputs, axis=0))
        if num_view > 1:
            for k in range(1, num_view):
                outputs = input_adj[..., k]
                outputs = ClsPositionEncodingLayer(
                    input_channel=input_shape[1], kenerl_size=kernel_size[0], strides=stride_size[0], d_model=input_shape[1], dropout_rate=dropout_rate, name='CLS_pos_encoding_' + str(k+1))(outputs)
                # outputs = ClsPositionEncodingLayer(
                #     input_channel=input_shape[1], kenerl_size=kernel_size[0], strides=stride_size[0], d_model=d_model, dropout_rate=dropout_rate, name=f'CLS_pos_encoding_{depth}')(outputs)
                outputs = Transformer(dropout_rate,
                                      n_layers,
                                      FFN_units,
                                      n_heads,
        ...
        outputs = tf.reshape(outputs, (num_view, -1, d_model * (channel+1)))
        outputs = layers.LSTM(
            units=lstm_unit, return_sequences=True, return_state=False)(outputs)
