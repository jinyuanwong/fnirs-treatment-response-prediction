...
class Classifier_Transformer():
    def __init__(self, output_directory, callbacks, input_shape, epochs, sweep_config, info):
        ...
        # sweep_config['FFN_units']  # random.choice([64, 128, 256, 512])  # 512, 64, 128,
        FFN_units = 512
        ...
        output_1 = ClsPositionEncodingLayer(
            input_channel=input_shape[1], kenerl_size=kernel_size[0], strides=stride_size[0], d_model=d_model, dropout_rate=dropout_rate, name='CLS_pos_encoding_1')(inputs)
        output_2 = ClsPositionEncodingLayer(
            input_channel=input_shape[1], kenerl_size=kernel_size[1], strides=stride_size[1], d_model=d_model, dropout_rate=dropout_rate, name='CLS_pos_encoding_2')(inputs)
