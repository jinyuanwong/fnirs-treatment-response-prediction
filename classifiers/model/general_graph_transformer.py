...
def graph_scaled_fot_product_attention(queries, keys, values, graph):

    product = tf.matmul(queries, keys, transpose_b=True)
    key_dim = tf.cast(tf.shape(keys)[-1], tf.float32)
    scaled_product = product / tf.math.sqrt(key_dim)
    # Apply the adjacency matrix as a mask
    # The mask is applied by setting the masked positions to a large negative value
    mask = graph - 1  # Convert 1s to 0s and 0s to -1s
    large_negative = tf.fill(tf.shape(mask), 1e9)
    mask = mask * large_negative  # Only non-edges (previously 0s) become a large negative value
    
    # Add the mask to the scaled product
    scaled_product += mask
...
class MultiHeadAttention(layers.Layer):
    ...
    def call(self, queries, keys, values, graph):
        input_shape = tf.shape(queries)
        batch_size = tf.shape(queries)[0]
        queries = self.query_lin(queries)
        keys = self.key_lin(keys)
        values = self.value_lin(values)
        # print("1 queries: ", queries)

        if self.n_heads > 1:
            queries = self.split_proj(queries, batch_size)
            keys = self.split_proj(keys, batch_size)
            values = self.split_proj(values, batch_size)
        # print("2 queries: ", queries)

        attention = graph_scaled_fot_product_attention(queries, keys, values, graph)
        # print("1-attention: ",attention)
        if self.n_heads > 1:
            attention = tf.transpose(attention, perm=[0, 2, 1, 3])
        attention = tf.reshape(attention,
                                      shape=(batch_size, input_shape[-2], self.d_model))
        
        # print("2-attention: ",concat_attention)
...
class EncoderLayer(layers.Layer):
    ...
    def call(self, inputs, adj_matrix):
        attention = self.multi_head_attention(inputs, inputs, inputs, adj_matrix)
...
class Encoder(layers.Layer):
    ...
    def call(self, inputs, graph):
        ...
        for i in range(self.n_layers):
            outputs = self.enc_layers[i](outputs, graph)
        return outputs
...
class GraphTransformer(tf.keras.Model):
    ...
    def call(self, inputs, graph):
        ...
        output_1 = self.encoder(inputs, graph)  # self.encoder_1(output_1)
