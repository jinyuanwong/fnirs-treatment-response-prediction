...
class Classifier_GNN_Transformer():
    def __init__(self, output_directory, callbacks, input_shape, epochs, sweep_config, info):
        ...
        inputs_FC = tf.keras.Input(shape=(input_shape[1], input_shape[1]))
        inputs_selected_features = tf.keras.Input(shape=(input_shape[1], input_shape[1]))
        ...
        model = tf.keras.Model(inputs=[inputs_time_point, input_adj], outputs=outputs)
        model.summary()
